...
--[[
    Opens the DaVinci Resolve Fusion Scripts root folder in the system
    file browser (Finder / Explorer / xdg-open).
...
-- Levels above this file's directory: Tools -> EditorScripts -> Utility -> Scripts
local LEVELS_UP = 3
...
local function getTargetPath()
    ...
    if fusion and fusion.MapPath then
        local mapped = fusion:MapPath("Scripts:/")
        if mapped and mapped ~= "" and mapped ~= "Scripts:/" then
            return mapped
        end
    end
    ...
    if package.config:sub(1, 1) == "\\" then
        ...
        if programData then
            return programData .. "\\Blackmagic Design\\DaVinci Resolve\\Fusion\\Scripts"
        end
        return nil
    end
    local macPath = "/Library/Application Support/Blackmagic Design/DaVinci Resolve/Fusion/Scripts"
    if pathExists(macPath) then
        return macPath
    end
    return "/var/BlackmagicDesign/DaVinci Resolve/Fusion/Scripts"
end
...
if target then
    ...
    print("[" .. SCRIPT_INFO.NAME .. "] Could not locate the Fusion Scripts folder.")
end
